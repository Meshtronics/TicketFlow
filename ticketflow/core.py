...
import re, os, sys, shutil, subprocess, webbrowser
...
def _slug(txt: str) -> str:
    return "".join(c.lower() if c.isalnum() else "-" for c in txt).strip("-").replace("--", "-")
...
def create_ticket(title: str, *, open_in_editor: bool = True, github_issue: bool | None = None) -> Path:
    ...
    if github_issue:
        ...
        if shutil.which(cli):
            ...
            subprocess.run(
                [cli, "issue", "create", "--title", f"{ticket_id} {title}",
                 "--label", labels, "--body", f"See `{ticket_id}` in repo /tickets."],
                check=False,
            )
